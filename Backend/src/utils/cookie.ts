...
const MFA_PENDING_COOKIE_OPTIONS = "HttpOnly; Path=/; SameSite=Strict; Max-Age=300" as const;
...
export function buildMfaPendingCookie(token: string): string {
   return `mfa_pending=${token}; ${MFA_PENDING_COOKIE_OPTIONS}`;
}

export function clearMfaPendingCookie(): string {
   return `mfa_pending=; HttpOnly; Path=/; SameSite=Strict; Max-Age=0`;
}
